import requests
import xml.etree.ElementTree as ET
import logging
import sys
import time
import traceback
from selenium import webdriver
from selenium.webdriver.chrome.options import Options

# ロガーの基本設定
logging.basicConfig(stream=sys.stdout, level=logging.INFO, format='%(asctime)s [%(levelname)s] %(message)s')

SITEMAP_URL = "https://w.atwiki.jp/arifure/sitemaps.xml"

def setup_driver():
    """Koyebの環境でSeleniumを動かすための設定"""
    chrome_options = Options()
    options = [
        "--headless", "--no-sandbox", "--disable-dev-shm-usage", "--disable-gpu", "--window-size=1920,1200",
        "--user-agent=Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/108.0.0.0 Safari/537.36",
        "--disable-extensions", "--remote-debugging-port=9222"
    ]
    for option in options:
        chrome_options.add_argument(option)
    
    driver = webdriver.Chrome(options=chrome_options)
    driver.set_page_load_timeout(60)
    return driver

def run_script():
    logging.info("--- Access Booster Script Started on Koyeb ---")
    driver = None
    try:
        sitemap_res = requests.get(SITEMAP_URL, timeout=30)
        sitemap_res.raise_for_status()
        root = ET.fromstring(sitemap_res.content)
        namespace = {'sitemap': 'http://www.sitemaps.org/schemas/sitemap/0.9'}
        urls = [elem.text for elem in root.findall('sitemap:url/sitemap:loc', namespace)]
        logging.info(f"Found {len(urls)} URLs. Starting browser access.")

        driver = setup_driver()

        for i, url in enumerate(urls, 1):
            try:
                logging.info(f"-> Visiting page {i}/{len(urls)}: {url}...")
                driver.get(url)
                time.sleep(5)
                logging.info(f"  -> Successfully visited.")
            except Exception:
                logging.error(f"  -> Failed to visit {url}:\n{traceback.format_exc()}")
        
        logging.info("--- All pages processed. ---")

    except Exception:
        logging.error(f"An unexpected error occurred:\n{traceback.format_exc()}")
    finally:
        if driver:
            driver.quit()
        logging.info("--- Script Finished. ---")

if __name__ == "__main__":
    run_script()import requests
import xml.etree.ElementTree as ET
import logging
import sys
import time
import traceback
from selenium import webdriver
from selenium.webdriver.chrome.options import Options

# ロガーの基本設定
logging.basicConfig(stream=sys.stdout, level=logging.INFO, format='%(asctime)s [%(levelname)s] %(message)s')

SITEMAP_URL = "https://w.atwiki.jp/arifure/sitemaps.xml"

def setup_driver():
    """Koyebの環境でSeleniumを動かすための設定"""
    chrome_options = Options()
    options = [
        "--headless", "--no-sandbox", "--disable-dev-shm-usage", "--disable-gpu", "--window-size=1920,1200",
        "--user-agent=Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/108.0.0.0 Safari/537.36",
        "--disable-extensions", "--remote-debugging-port=9222"
    ]
    for option in options:
        chrome_options.add_argument(option)
    
    driver = webdriver.Chrome(options=chrome_options)
    driver.set_page_load_timeout(60)
    return driver

def run_script():
    logging.info("--- Access Booster Script Started on Koyeb ---")
    driver = None
    try:
        sitemap_res = requests.get(SITEMAP_URL, timeout=30)
        sitemap_res.raise_for_status()
        root = ET.fromstring(sitemap_res.content)
        namespace = {'sitemap': 'http://www.sitemaps.org/schemas/sitemap/0.9'}
        urls = [elem.text for elem in root.findall('sitemap:url/sitemap:loc', namespace)]
        logging.info(f"Found {len(urls)} URLs. Starting browser access.")

        driver = setup_driver()

        for i, url in enumerate(urls, 1):
            try:
                logging.info(f"-> Visiting page {i}/{len(urls)}: {url}...")
                driver.get(url)
                time.sleep(5)
                logging.info(f"  -> Successfully visited.")
            except Exception:
                logging.error(f"  -> Failed to visit {url}:\n{traceback.format_exc()}")
        
        logging.info("--- All pages processed. ---")

    except Exception:
        logging.error(f"An unexpected error occurred:\n{traceback.format_exc()}")
    finally:
        if driver:
            driver.quit()
        logging.info("--- Script Finished. ---")

if __name__ == "__main__":
    run_script()
